...
def get_model_capabilities(
    ...
    models = _get_provider_models(catalog_provider, allow_network=allow_network, config=config)
    entry = _find_model_entry(models, catalog_model, catalog_provider) if models is not None else None
    unknown_base = (
        entry is None
        and _builtin_model_metadata(catalog_provider, catalog_model, config=config) is None
    )
    if catalog_provider != provider and _explicit_model_override(provider, model, config=config) is None:
        # No SDK-keyed override: the catalog identity's overrides (``model_overrides.anthropic``) apply.
        raw = _apply_overrides(catalog_provider, catalog_model, entry, config=config)
    else:
