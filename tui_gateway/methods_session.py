...
def _create_session(rid, params: dict, *, copy_parent_history: bool = False) -> dict:
    ...
    # Resolved inside the session's own profile scope (equivalent to upstream's _session_default_model),
    # and provider is always reported (cntrl W1: the SDK engine is the desktop default).
    with _profile_build_scope(profile_home):
