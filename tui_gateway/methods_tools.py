...
def _refresh_live_sessions(
    home=None, *, preserve_prefix: bool = False, note: str = "", sdk_rotate: bool = False,
) -> None:
...
def _(rid, params: dict) -> dict:
    ...
    def _refresh_session_agent() -> None:
        ...
        ``sdk_rotate``: an explicit /reload-mcp forces claude-agent-sdk sessions to rotate their CLI
        session so the reloaded MCP set is live (cntrl carry)."""
        _refresh_live_sessions(sdk_rotate=True)
...
def _catalog_skills(cat: _Catalog, skills: dict[str, dict]) -> str:
    ...
    commands = _tools_mod("agent.claude_sdk_slash").merged_skill_commands(sc.scan_skill_commands())
