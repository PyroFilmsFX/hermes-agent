...
async def rename_session_endpoint(session_id: str, body: SessionRename):
    ...
    result = await asyncio.to_thread(_with_db, body.profile, _update, read_only=False)
    if body.title is not None and isinstance(result, dict):
        # Off the event loop, as the sync endpoint ran before upstream made it async.
        await asyncio.to_thread(_rename_live_sdk_session, str(result.pop("_sid", "") or ""))
    return result
