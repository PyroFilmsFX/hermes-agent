...
def _handle_create(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        # cntrl carry (c0deb07894): once a session context is engaged, an unset session id must not
        # fall back to os.environ at all (the gateway's env is another session's).
        ambient_sid = ""
        try:
            ...
            if val is not _UNSET and val:
                ...
                ambient_sid = get_session_env("HERMES_SESSION_ID", "")
        except Exception:
            ambient_sid = get_session_env("HERMES_SESSION_ID", "")
        session_id = (_persisted_session_id(args.get("session_id"))
                      ...
                      or _persisted_session_id(ambient_sid))
