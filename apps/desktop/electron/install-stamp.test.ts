...
describe('loadInstallStamp in production', () => {
  it('reports the build stamp from resourcesPath when packaged', () => {
    const fsImpl = fakeFs({
      '/resources/install-stamp.json': JSON.stringify(STALE_BUILD_STAMP)
    })
    ...
    expect(stamp?.commit).toBe(STALE_BUILD_STAMP.commit)
    expect(stamp?.source).toBe('local')
  })

  it('reports the build stamp from build/install-stamp.json when resourcesPath is missing', () => {
    const fsImpl = fakeFs({
  ...
  it('returns null when no stamp file exists in production', () => {
    const fsImpl = fakeFs({})
  ...
  it('ignores stamps with mismatched schemaVersion', () => {
    const fsImpl = fakeFs({
      '/resources/install-stamp.json': JSON.stringify({ ...STALE_BUILD_STAMP, schemaVersion: 99 })
    })
...
describe('loadInstallStamp in DEV mode', () => {
