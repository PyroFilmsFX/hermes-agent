...
export async function desktopSessionCreateParams(
  ...
  return {
    ...
    ...(includeComposerSelection
      ? {
          // cntrl carry (W1): never send a model without its provider; the gateway resolves the
          // configured provider instead of inferring plain anthropic.
          ...(selection.model && selection.provider ? { model: selection.model, provider: selection.provider } : {}),
