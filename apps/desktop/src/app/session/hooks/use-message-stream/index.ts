  ...
  const completeAssistantMessage = useCallback(
    (
      ...
      const completedState = updateSessionState(sessionId, state => {
        ...
        // cntrl carry: an SDK background-lane completion settles onto the bubble
        // that carries its delivery id, wherever it sits (a later user message
        // does not make it a new occurrence).
        const deliveryIndex =
          streamIndex < 0 && effectiveDeliveryId
            ? prev.findLastIndex(
                message =>
        ...
        if (streamIndex >= 0) {
          ...
          nextMessages = settleAt(deliveryIndex)
        } else {
